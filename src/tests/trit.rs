...
#[test]
fn trit_into_char() {
    assert_eq!('T', Trit(Trit::BIN_NEG).into());
    assert_eq!('0', Trit(Trit::BIN_ZERO).into());
    assert_eq!('1', Trit(Trit::BIN_POS).into());
}

#[test]
fn trit_from_char() {
    assert_eq!(Trit::BIN_NEG, Trit::try_from('T').unwrap().0);
    assert_eq!(Trit::BIN_ZERO, Trit::try_from('0').unwrap().0);
    assert_eq!(Trit::BIN_POS, Trit::try_from('1').unwrap().0);
}
